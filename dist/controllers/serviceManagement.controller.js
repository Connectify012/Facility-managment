...
class ServiceManagementController {
    ...
    static async initializeServices(req, res, next) {
        try {
            ...
            const existingServices = await ServiceManagement_1.ServiceManagement.findOne({ facilityId, isDeleted: false });
            if (existingServices) {
                throw new errorHandler_1.AppError('Services already initialized for this facility', 400);
            }
            // Initialize default services
            const serviceManagement = await ServiceManagement_1.ServiceManagement.initializeDefaultServices(new mongoose_1.default.Types.ObjectId(facilityId), facilityName, facilityType, new mongoose_1.default.Types.ObjectId(createdBy));
            res.status(201).json({
                status: 'success',
            ...
            logger_1.logger.error('Initialize services error:', error);
            next(error);
        }
    }
    // Get services by facility ID
    static async getServicesByFacilityId(req, res, next) {
        try {
            ...
            let services = await ServiceManagement_1.ServiceManagement.findOne({
                facilityId,
                isDeleted: false
            }).lean();
            ...
            if (!services) {
                try {
                    logger_1.logger.info(`Services not found for facility ${facilityId}, attempting auto-initialization`);
                    ...
                    const initializedServices = await ServiceManagementController.autoInitializeServicesForFacility(facilityId, facility.siteName, facility.facilityType, facilityId // Use facility ID as createdBy since we don't have user context
                    );
                    // Fetch the newly created services
                    services = await ServiceManagement_1.ServiceManagement.findOne({
                        facilityId,
                        isDeleted: false
                    }).lean();
                    if (!services) {
                        throw new errorHandler_1.AppError('Failed to initialize services for this facility', 500);
                    }
                    logger_1.logger.info(`Services auto-initialized successfully for facility ${facilityId}`);
                }
                catch (initError) {
                    logger_1.logger.error(`Failed to auto-initialize services for facility ${facilityId}:`, initError);
                    throw new errorHandler_1.AppError('Services not found for this facility and could not be auto-initialized', 404);
                }
            }
            // Filter by category if specified
            let filteredServices = services;
            if (category && Object.values(ServiceManagement_1.ServiceCategory).includes(category)) {
                filteredServices = {
                    ...services,
            ...
            logger_1.logger.error('Get services by facility ID error:', error);
            next(error);
        }
    }
    ...
    static async getAllServices(req, res, next) {
        try {
            const { search, facilityType, category, sortBy = 'lastUpdated', sortOrder = 'desc' } = req.query;
            ...
            const [services, totalCount] = await Promise.all([
                ServiceManagement_1.ServiceManagement.find(filter)
                    .sort(sort)
                    .skip(skip)
                    .limit(limit)
                    .lean(),
                ServiceManagement_1.ServiceManagement.countDocuments(filter)
            ]);
            ...
            logger_1.logger.error('Get all services error:', error);
            next(error);
        }
    }
    // Add a new service to a category
    static async addServiceToCategory(req, res, next) {
        try {
            const { facilityId } = req.params;
            ...
            if (!Object.values(ServiceManagement_1.ServiceCategory).includes(category)) {
                throw new errorHandler_1.AppError('Invalid service category', 400);
            }
            const serviceManagement = await ServiceManagement_1.ServiceManagement.findOne({
                facilityId,
                isDeleted: false
            });
            if (!serviceManagement) {
                throw new errorHandler_1.AppError('Services not found for this facility', 404);
            }
            ...
            if (existingCategory) {
                ...
                if (existingService) {
                    throw new errorHandler_1.AppError(`Service '${name}' already exists in ${category}`, 400);
                }
            }
            ...
            serviceManagement.updatedBy = new mongoose_1.default.Types.ObjectId(updatedBy);
            ...
            logger_1.logger.error('Add service to category error:', error);
            next(error);
        }
    }
    ...
    static async updateServiceStatus(req, res, next) {
        try {
            ...
            logger_1.logger.error('Update service status error:', error);
            next(error);
        }
    }
    // Update service details (name, description)
    static async updateServiceDetails(req, res, next) {
        try {
            const { facilityId } = req.params;
            ...
            if (!service) {
                throw new errorHandler_1.AppError(`Service ${oldServiceName} not found in category ${category}`, 404);
            }
            ...
            if (newServiceName && newServiceName !== oldServiceName) {
                ...
                if (existingService) {
                    throw new errorHandler_1.AppError(`Service '${newServiceName}' already exists in ${category}`, 400);
                }
            ...
            logger_1.logger.error('Update service details error:', error);
            next(error);
        }
    }
    // Remove service from category
    static async removeServiceFromCategory(req, res, next) {
        try {
            const { facilityId } = req.params;
            ...
            if (serviceIndex === -1) {
                throw new errorHandler_1.AppError(`Service ${serviceName} not found in category ${category}`, 404);
            }
            // Remove the service
            ...
            logger_1.logger.error('Remove service from category error:', error);
            next(error);
        }
    }
    // Bulk update services
    static async bulkUpdateServices(req, res, next) {
        try {
            const { facilityId } = req.params;
            ...
            if (!Array.isArray(services) || services.length === 0) {
                throw new errorHandler_1.AppError('Please provide an array of services to update', 400);
            }
            const serviceManagement = await ServiceManagement_1.ServiceManagement.findOne({
                facilityId,
                isDeleted: false
            ...
            for (const serviceUpdate of services) {
                ...
                if (!Object.values(ServiceManagement_1.ServiceCategory).includes(category)) {
                    throw new errorHandler_1.AppError(`Invalid service category: ${category}`, 400);
                }
                try {
                    await serviceManagement.updateServiceStatus(category, serviceName, isActive);
                }
                catch (error) {
                    logger_1.logger.warn(`Failed to update service ${serviceName} in ${category}:`, error);
                }
            }
            serviceManagement.updatedBy = new mongoose_1.default.Types.ObjectId(updatedBy);
            await serviceManagement.save();
            ...
            logger_1.logger.error('Bulk update services error:', error);
            next(error);
        }
    }
    ...
    static async getServiceStatistics(req, res, next) {
        try {
            ...
            const services = await ServiceManagement_1.ServiceManagement.findOne({
                facilityId,
                isDeleted: false
            }).lean();
            if (!services) {
            ...
            logger_1.logger.error('Get service statistics error:', error);
            next(error);
        }
    }
    // Delete service management (soft delete)
    static async deleteServiceManagement(req, res, next) {
        try {
            const { facilityId } = req.params;
            ...
            const serviceManagement = await ServiceManagement_1.ServiceManagement.findOneAndUpdate({ facilityId, isDeleted: false }, {
                isDeleted: true,
            ...
            logger_1.logger.error('Delete service management error:', error);
            next(error);
        }
    }
    // Get global service statistics (across all facilities)
    static async getGlobalServiceStatistics(req, res, next) {
        try {
            const [totalFacilities, servicesByCategory, servicesByFacilityType, mostActiveServices] = await Promise.all([
                ServiceManagement_1.ServiceManagement.countDocuments({ isDeleted: false }),
                ServiceManagement_1.ServiceManagement.aggregate([
            ...
            logger_1.logger.error('Get global service statistics error:', error);
            next(error);
        }
    }
    // Debug endpoint to check facility and service status
    static async debugFacilityServices(req, res, next) {
        try {
            const { facilityId } = req.params;
            if (!(0, validation_1.validateObjectId)(facilityId)) {
                throw new errorHandler_1.AppError('Invalid facility ID', 400);
            ...
            const allServices = await ServiceManagement_1.ServiceManagement.find({ facilityId }).lean();
            res.json({
                status: 'success',
                data: {
            ...
            logger_1.logger.error('Debug facility services error:', error);
            next(error);
        }
    }
    // Helper function to auto-initialize services for a facility (used internally)
    static async autoInitializeServicesForFacility(facilityId, facilityName, facilityType, createdBy) {
        try {
            // Check if services already exist for this facility
            const existingServices = await ServiceManagement_1.ServiceManagement.findOne({
                facilityId: new mongoose_1.default.Types.ObjectId(facilityId.toString()),
                isDeleted: false
            });
            if (existingServices) {
                logger_1.logger.info(`Services already exist for facility: ${facilityId}`);
            ...
            // Initialize default services
            const serviceManagement = await ServiceManagement_1.ServiceManagement.initializeDefaultServices(new mongoose_1.default.Types.ObjectId(facilityId.toString()), facilityName, facilityType, createdById);
            logger_1.logger.info(`Services auto-initialized for facility: ${facilityId}`);
            return serviceManagement;
        }
        catch (error) {
            logger_1.logger.error(`Failed to auto-initialize services for facility ${facilityId}:`, error);
            throw error;
        }
    }
}
